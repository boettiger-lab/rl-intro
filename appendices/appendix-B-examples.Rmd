...
For example, here we will try the Soft Actor-Critic algorithm, SAC, training with the default hyperparameters originally posed in @SAC.  We use SAC to train an agent that has a multi-layer perceptron ('MlpPolicy') neural network, with the default architecture of two 64-neuron layers.
...
Having saved our trained agent to disk (`cache/sac.zip`), we can then re-load this agent for evaluation or to continue training.
...
agent = sb3$SAC$load("../python/cache/sac")
```
...
After an initial over-harvest (top panel), SAC achieves nearly optimal performance as measured by reward over time, (bottom right), or compared to the steady-state management solution.
The policy function (bottom left) explains this behavior.
The RL agent has a nearly optimal policy for stock sizes between zero and just above the $B_{MSY}$ target.
At higher stocks, the agent has a tendency to over-harvest.




In this simple example, SAC is able to achieve nearly-optimal performance (as defined by the reward conditions) without tuning, simply by using hyperparameters originally chosen for different RL tasks.
As noted in appendix C, careful design of the environment, such as rescaling of state and action space, can also improve the ability of a Deep RL algorithm to find an acceptable policy without altering the substance of the challenge.
Even so, our agent still had extensive time to train on this specific environment, a total of 300,000 timesteps or 3000 training episodes.
Sufficient tuning of the hyperparameters to this environment may be able result in better performance than we have seen here.
For more difficult problems, it may be impossible to find an acceptable policy without first tuning candidate approaches to select the best hyperparameters for the environment.
...
# parameters based on statistical best model estimate, see Memarzadeh & Boettiger 2019
# or Appendix D
agent = sb3$SAC$load("../python/cache/sac")
...
Tmax = 15
years =1986:2000
agent_state = agent_action = numeric(Tmax)
agent_state[1] = x0
agent_action[1] = NA
```


Our hindcast considers the scenario of managing catch quotas using the RL agent, beginning in 1986. 
We compare both the anticipated harvest and resulting stock biomass to the historically recorded harvest and biomass.
This shows that quotas set by the RL agent would have been lower than historical values throughout the late 80s and early 90s, which
saw a sharp decline in estimated biomass of the Argentine Hake.
For the same or higher estimated stock sizes from the historical record, we see that the RL agent would recommend a lower quota.
We also see that under the RL agent the stock recovers sufficiently to predict higher quotas by the second half of the 90s than were
being set in the now-depleted stock.  
Whether the the RL agent quotas would have truly been sufficient to permit recovery in stock sizes shown in the simulation clearly depends on the accuracy of the underlying simulation.
However, the fact that the RL agent would set a lower quota than the historical management given the same or even higher estimate of stock size is independent of the Hake simulation.
...
```{r}
# Env is used only for helper methods to translate between original units and transformed space
env = gym$make("fishing-v1")
## Now we can represent the initial stock size in the 'rescaled' state space.
...
df = bind_rows(
  tibble(year = years, state = agent_state, action = agent_action, model = "RL"),
