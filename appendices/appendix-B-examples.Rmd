...
colors <- set_names(c(pal[c(1,3,8,2)], txtcolor, pal[6]),
                    c("Optimal", "TD3", "TD3_tuned", "RL", "historical", "steady-state"))
...
TD3_sims = env$simulate(model, reps = 100L) %>% mutate(model = "TD3_tuned")
TD3_policy = env$policyfn(model, reps = 1L)  %>% mutate(model = "TD3_tuned")
```
