...
For example, here we will try the A2C actor-critic algorithm, an extension of the A3C algorithm originally described in @A2C.
We train an agent using the A2C algorithm using default hyper-parameter settings (which correspond to those used in @A2C),
using a multi-layer perceptron ('MlpPolicy') using the default architecture for the policy network and value network composed of two 64-neuron layers.
...
Having saved our trained agent to disk (`cache/a2c.zip`), we can then re-load this agent for evaluation or to continue training.
...
for(i in 1:10){
  ...
  out = a2c$predict(state, deterministic=TRUE)
  action = out[[1]]
...
sims_df <- a2c_sims %>% 
  mutate(model = factor("A2C", algos)) %>% 
  bind_rows(sims_df)
policy_df <- a2c_policy  %>% 
  mutate(model = factor("A2C", algos))  %>% 
  bind_rows(policy_df)
...
After an initial over-harvest (top panel), A2C achieves nearly optimal performance as measured by reward over time, (bottom right), or compared to the steady-state management solution.
...
In this simple example, A2C is able to achieve nearly-optimal performance (as defined by the reward conditions) without additional tuning.
It is worth noting that this is not true of all RL algorithms.
For example, the PPO algorithm [@A2C] is a somewhat more computationally intensive alternative that combines strategies from A2C and another algorithm, TRPO [@PPO].
Nevertheless, using default hyper-parameters, PPO does considerably less well (not shown).
For harder problems, it is often necessary to consider a range of algorithms, and tune the hyper-parameters extensively before training will be able to discover an acceptable solution.
We illustrate this briefly in our second example gym. 
...
agent = sb3$A2C$load("../python/cache/a2c_untuned")
