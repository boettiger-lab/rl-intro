...
<!-- For example, here we will try the Actor-Critic algorithm A2C, training with the default hyperparameters originally posed in @A2C.-->
We use this algorithm to train an agent that has a multi-layer perceptron ('MlpPolicy') neural network, with the default architecture of two 64-neuron layers.
...
In this simple example, the RL agent is able to achieve nearly-optimal performance (as defined by the reward conditions) without tuning, simply by using hyperparameters originally chosen for different RL tasks.
