...
colors <- set_names(c(pal[c(1,3,5,8)], txtcolor),
                    c("Optimal", "TD3", "TD3_tuned", "RL", "historical"))
...
As we can see in the figure above, without hyper-parameter tuning, our agent has found only a sub-optimal policy.
...
By tuning the hyper-parameters that control the training regime used by the TD3 algorithm, we are able to further improve upon on the performance of the RL agent.
...
We train this new RL agent using it's tuned hyperparameters, and saving the resulting trained agent as before:
...
colors <- set_names(c(pal[c(1,3,8,2)], txtcolor),
                    c("Optimal", "TD3", "TD3_tuned", "RL", "historical"))
scale_colour_discrete <- function(...) scale_colour_manual(..., values=colors)
scale_fill_discrete <- function(...) scale_fill_manual(..., values=colors)
plot_sims(sims_df) / ( plot_policy(policy_df) + plot_reward(sims_df))
...
This pattern is not surprising, since these large stock values are least likely to observed over the simulated episodes.
For the same reason, that deviation contributes very little to reward. 
...
For each year in the data, we compare the historical harvest to the quota that would have been recommended by the TD3-based Deep RL agent we have just trained.
...
years =1986:2000
df = bind_rows(
  tibble(year = years, harvest = agent_action, model = "TD3_tuned"),
  tibble(year = years, harvest = hake$scaled_catch, model = "historical")
)
stock <-  tibble(year = years, biomass = hake$scaled_biomass, model = "historical")

top_panel <- ggplot(stock, aes(year, biomass, col=model)) + geom_line(lty=2) + 
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
bottom_panel <- ggplot(df, aes(year, harvest, col=model)) + 
  geom_line() + geom_point()

top_panel / bottom_panel
```

The Argentine hake biomass exhibited a steep decline between 1986 and 2000 as shown by the stock assessment estimates.
...
While this example shows that it is in principle very straight forward to utilize a trained RL agent to recommend real harvest quotas, we must bear in mind that the agent is only as good as the environments on which it was trained.
