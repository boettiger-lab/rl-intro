...
The aim here is not to give full treatment of all deep RL methods -- @suttonbarto is an excellent resource for theoretical underpinnings while the white papers behind the algorithms go into greater detail on modern approaches --; instead, this section is meant to serve as an abbreviated background view on model-free deep RL.
We hope to introduce some of the important design choices that are made for model-free deep RL algorithms and will point to specific resources that give a more complete presentation.
...
In this section, we will go over these algorithm classes to convey a broad sense of what they are attempting to achieve and the trade-offs between them.
...
The benefit of using neural networks to approximate Q-functions is that neural networks are able to generalize Q functions over unseen state-action pairs, whereas classic methods like TD-learning are not able to generalize [@DQN].
...
Yet, how the agent selects these actions is very important because if an agent fails to explore the state-action space well, the agent will likely learn a bad policy.
...
We will not go over these more advanced exploration algorithms but a good example is seen in @go-explore.
...
The main issue that arises from sampling past experiences is correlation of samples [@DQN].
...
Learning from correlated samples is undesirable since this can result in agents getting stuck in local optima, which will impede learning.
There are two commonly used solutions to this problem of correlated samples: Replay Buffers and Parallel Workers.
If the agent is off-policy, the agent can employ a replay buffer.
...
Since the replay buffer can be arbitrarily large and thus store transitions from numerous trajectories, sampling from the buffer will likely produce transitions that cover a broader region in state-action space than if the agent were to sample from one trajectory.
On-policy agents, however, cannot use a replay buffer since these agents are not allowed to use samples from a policy different than the policy they are trying to improve -- note that in a replay buffer, it is possible to have samples from different policies.
...
The on-policy agent will collect samples across these workers in a learning step, and then repeat the process of collecting and sampling transitions with an updated policy.
Once again, the idea behind using parallel workers is that sampling over multiple trajectories will de-correlate samples.
...
In this section, we briefly introduced some of the major design decisions that are made with model-free deep RL algorithms, but there are many more decisions that are made with current state of the art algorithms.
If you are interested in learning more about model-free deep RL, then reading papers like @sac, @DQN and @ppo, are good places to start.
We have also not given any space to discuss model-based or gradient-free techniques which also warrant attention -- see @mbpo and @salimans_evolution_2017 for more here.
A benefit of model-free deep RL over these other approaches is that there has been more of a general focus on model-free deep RL, so there are widely available packages like Stable Baselines and TensorFlow Agents which implement model-free deep RL; but the focus on model-free algorithm could easily change in coming years.
Deep RL has only become established in the last decade, and with its amount of unanswered research questions, deep RL may evolve significantly in the near future.
