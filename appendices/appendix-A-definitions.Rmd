...
On-policy agents, however, cannot use a replay buffer since these agents are not allowed to use samples from a policy different than the policy they are trying to improve -- replay buffers are generally large enough to hold transitions from old policies.
...
The idea behind using parallel workers is that sampling over multiple trajectories will de-correlate samples -- see @a3c on more details here.
