...
We train the A2C algorithm using default hyper-parameter settings, based on the original publication [@A2C; @SB3].  While default hyperparameters provide a useful starting place (particularly when the environment has been suitably normalized, a best-practice we discuss above), better performance can almost always be achived by _tuning_ the hyperparameter selection. 
...
Hyperparameters are selected through the process of model tuning, see.  
...
hyper['n_episodes_rollout'] = -1 # deprecated in latest SB3
