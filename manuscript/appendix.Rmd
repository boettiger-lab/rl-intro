...
Development of any deep reinforcement learning application can be divided into two components: an _environment_ and an _agent_.
...
An agent is typically a computational neural network trained using an appropriate algorithm to suggest an action based on available information about the environment.
...
@brockman2016 identified (1) the lack of standardization of environments used in publication and (2) the need for better benchmark environments as two core problems limiting more rapid advancement of RL research, and proposed the creation of the OpenAI `gym` framework to address both poblems. 
