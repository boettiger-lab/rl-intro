...
@brockman2016 identified (1) the lack of standardization of environments used in publication and (2) the need for better benchmark environments as two core problems limiting more rapid advancement of RL research; to address these issues, @brockman2016 proposed the creation of the OpenAI `gym` framework.
...
The reinforcement learning problem is typically formalized as a discrete-time partially observable Markov decision process (POMDP).
...
After the agent makes an observation at time t, $o_t$, the agent selects an action, $a_t$, according to its policy, $\pi$, which maps observations to a probability distribution over actions, $\pi: \Omega \rightarrow p(\mathcal{A} \mid \Omega)$.
This action will cause the environment to transition to a new state according to its state transition operator. 
The agent will then receive a new observation and the process will repeat until a terminal state is reached.
...
This Markov property, which is often an unrealistic assumption, is held in most of the RL algorithms that we discuss in the next section.

In formal terms, the goal of reinforcement learning is to find an optimal policy distribution, $\pi^*(a_t|o_t)$, that maximizes the expected return, $J(\pi)$:
