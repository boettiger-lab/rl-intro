...
Yet, for many decision-making problems, the state-action space cannot be tractably represented in a table; one effective alternative for this case is to use a neural network to approximate the Q-function, a method called deep Q-learning. 
...
In the case where the Q-function is approximated by some neural network and thus represented as $Q_\theta$, the Bellman error for one state-action pair would be

$$
  \mathcal{E} = r(s_t, a_t) + \gamma \, \underset{a}{\text{max}} \, Q_\theta(s_{t+1}, a) - Q_\theta(s_t, a_t).
$$

See the appendix/reference for details on how the Bellman Error is derived from eq. [@CITE]. 
The general scheme for approximate value-learning with deep neural networks is to sample trajectories from the environment, evaluate the Bellman Error across a range of these samples then perform the following gradient descent,

$$
  \theta_{t+1} = \theta_t - \alpha \nabla_\theta \, \sum_i \mathcal{E}_i^2,
$$

where $i$ indexes the sample.
After convergence to the Q function, the optimal policy could 
