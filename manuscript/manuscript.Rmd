...
Even simpler simulators are frequently employed throughout ecology to guide understanding and response. <!-- ML: I think this sentence could be dropped -->
...
In this paper, we draw on examples from fisheries management, ecological tipping points, and wildfire spread to illustrate how deep RL techniques can successfully discover optimal solutions to previously solved management scenarios and discover highly effective solutions to unsolved problems. <!-- ML: If we are dropping wildfire example, we'll need to cut wildfire mentions here-->
...
<!-- ML: I have some qualms about this figure; I think it can do a bit more to display what is going on. Like perhaps can put dashed arrows within a trial to show that there is continuity between episodes but not continuity across trials. We could also put a dashed box around the trials to this effect as well. In these dashed boxes, we could put smaller hyperparameter boxes to convey that different trials have distinct hyperparameters. I am also not a fan of the object used for neural networks. -->
\begin{figure}
...
In this brief section, we will introduce the concept of the RL environment as partially observable Markov decision process (POMDP); we will then use POMDP definitions to define the RL objective.
Since all RL algorithms are either directly or indirectly maximizing the RL objective, understanding the RL objective is critical to understanding how RL algorithms work. <!-- ML: this seems like an obvious sentence to me but am stuck on it atm so revisions welcomed-->
...
In table 1, we provide some of the commonly used RL algorithms and delineate some of their important characteristics. <!-- ML: Have we dropped the table? -->
