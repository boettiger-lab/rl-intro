...
Applications of ML in ecology have begun to illustrate the promise of two methods of ML: *supervised learning* [@joseph_neural_2020] and *unsupervised learning* [@valletta_applications_2017], but have so far largely overlooked the third and possibly most promising approach in the ML triad: *reinforcement learning* (RL).
...
An *environment* is typically a computer simulation, though it is possible to use the real world as the RL environment [@ha_learning_2020].
...
We will not discuss in detail how neural networks work [see @lecun_deep_2015], but the general process for how neural networks are used in RL is that at each learning step, the agent, will adjust the parameters in its neural network with which it approximates the policy function and/or value function, so that according to past experiences, the agent will be more likely to engage in highly rewarding behavior in the future. 

On top of these distinctions of deep RL algorithms, there are other algorithms design choices to be made. 
