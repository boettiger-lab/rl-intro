...
An *environment* is typically a computer simulation, though it is possible to use the real world as the RL environment.
...
The distinction is that *model-free* algorithms do not attempt to learn or use a model of the environment.
Conversely, *Model-based* algorithms either learn or use a model of the environment to achieve the RL objective.
...
<!-- Write paragraph on classic approaches then compare to deep RL very broadly-->
...
Another important consideration in RL is whether to use an episodic or non-episodic environment.
In episodic environments, there is a finite number of time steps during which the agent interacts with the environment, and for these environments, the designer must specify a stopping condition.
Conversely, with non-episodic environments, the agent can interact with the environment ad infinitum. 
For this paper, we will only consider episodic environments.
...
Over the course of training, the agent will engage in a trial and error process whereby the agent continually updates weights on a collection of neural networks in response to the environment's reward signal, much like how reinforcement learning is believed to work in living creatures [@rl_brain].
Fundamental to this trial and error process is a balancing of explorative and exploitative behavior.
...
As the exploration-exploitation is currently unsolved, there are a range of approaches that RL algorithms employ.

Model-free algorithms can explore the environment in either an off-policy or on-policy manner. 
The distinction here comes from what policy the agent is using during training.
For off-policy algorithms, the agent is using a policy that can be different than the policy the agent will use during evaluation to select actions; while in on-policy algoirthms, the agent uses the same policy in training as the agent will use during evaluation.
On top of this distinction, agents can learn either a deterministic or stochastic policy. 
...
When the environment is fully observable, i.e. when the state that the agent observes fully characterizes the system, the reinforcement learning problem is conventionally formalized as a Markov Decision Process (MDP).
