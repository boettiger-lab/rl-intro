...
And, at evaluation, if you give an agent a novel task, including tasks that are closely related to the examples that the agent saw in training, the agent can easily fail to generalize its learned behavior to this new situation [@CITE].
...
Deep RL agents also suffer from an inability to be interpreted [@cite].
Deep neural networks are commonly referred to as black box models, since it is not possible to fully explain why a deep neural network produces a specific output from a given input [@cite].
For deep RL, this means that we cannot provide a meaningful explanation for why any agent selects an action.
Compounded with the previously mentioned generalizability issues, the lack of explainability introduces a lot of safety concerns when attempting to apply these methods to the real world.
When going from a simulator to the real world, the real world may introduce novel situations that a simulator-trained agent is not familiar with, and since we can't anticipate how an agent will respond to new situations, there is an inherent risk in letting RL agents make decisions in the real world. 
...
Thus, for ecological systems, which are typically difficult to control to begin with, specifying a reward function could easily produce behavior that opposes the intents of the RL practitioner.
To guide agents to learn desirable policies, RL researchers often *shape* the reward function so that the agent will more readily learn certain behaviors[@cite].
This approach is particularly common when rewards are *sparse*, which occurs when there are few state-action pairs that have strong feedback signals.
Frequently, when researchers do not shape the rewards on sparse reward problems, the RL agent learns nothing [@CITE], but reward shaping can still backfire and produce unintended agent behavior [@CITE]. 
Given the nature of many conservation problems, where benefits rarely come immediately and thus the reward structures are sparse, RL agents will likely have difficulty learning anything, or, in the case that researchers shape the rewards to aid learning, the agents may learn policies that are undesirable.
Moreover, specifying a reward function for conservation brings up various ethical concerns.
e.g., Who gets to specify the reward function, and will the values of the greater community be represented in the reward function?
