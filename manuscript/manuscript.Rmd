...
\caption{Deep Reinforcement Learning:  An artificial intelligence \emph{agent} uses a \emph{neural network} to select an \emph{action} in response to an \emph{observation} of the \emph{environment}, receiving a \emph{reward} (or punishment) from the environment as a result.  The agent is \emph{trained} to maximize the reward through the experience of trying different actions over repeated \emph{epsiodes} of interacting with the environment.  Over the course of training, the agent adjusts parameter weights on the neural network as it learns which actions lead to the best cumulative rewards. A researcher may \emph{tune} an agent by considering iterative trials that train different agents with different \emph{hyperparameters}, such as different neural network architectures.  An agent trained with the most successful combination of hyperparameters can then be tested in an evaluation environment.}
\end{figure}
...
The training period involves a trial and error process whereby the agent will perform an action and then receive feedback from the environment in the form of a reward. 
This process will repeat until the environment transitions to a terminal state, after which the environment will reset to an initial state.
According to the algorithm that the agent follows, the agent will explore the state-action space and use its experience to improve its policy.
...
An advantage of using deep, model-free algorithms is that since they function autonomously and do not require any knowledge about the environment, we can readily test how these algorithms perform without knowing much about the environment's transition dynamics or all the inner workings of the algorithm.

Hyperparameters refer to the parameters in a deep, RL algorithm that are not directly learned during the training process.
...
In this brief section, we will introduce the concept of the RL environment as a POMDP; we will then use POMDP definitions to define the RL objective.
Since all RL algorithms are either directly or indirectly maximizing the RL objective, understanding the RL objective is crucial to understanding how RL algorithms work. <!-- ML: this seems like an obvious sentence to me but am stuck on it atm so revisions welcomed-->
...
This process will repeat until a terminal state is reached.
...
And, lastly, *actor-critic* algorithms attempt to learn both a policy function and a value function, whereby the value function is used to inform the agent on the goodness of a selected action.
There is a handful of advantages and disadvantages for each of these approaches that we will not get into in depth -- for more, see the appendix; yet, the main point of divergence comes from the bias-variance trade-off [@suttonbarto].
Generally, policy gradient algorithms tend to struggle with high variance, value-based methods tends to be biased while actor-critic methods balance these two concerns [@suttonbarto; @qbias].
...
This *exploration-exploitation dilemma* is an open research problem [@berger-tal_exploration-exploitation_2014], and current deep RL algorithms take a range of approaches towards this dilemma -- in the appendix, we discuss some of the more specific exploration algorithms.
...
For off-policy algorithms, the agent uses a policy during training that can be different from the policy that the agent will use during evaluation; while in on-policy algorithms, the agent uses the same policy in training as the agent will use during evaluation.
...
\hline
Algorithm & Description & Model & Method & Evaluation  & Constraints  \\
\hline
MBPO & Model-based Policy Optimization [@mbpo] & Model-based & Planning & On-policy & \\
\hline
MCTS* & Monte Carlo Tree Search [@suttonbarto] & Model-based & Planning & Either & Not suited for continuous action and state spaces \\
\hline
A2C  & Advantage Actor Critic [@a2c] & Model-free & Actor Critic  & On-policy & \\
\hline
A3C  & Asynchronous A2C [@a3c] & Model-free & Actor Critic & On-policy & \\
\hline
ACKTR  &  Actor Critic using Kronecker-Factored Trust Region [@acktr] & Model-free & Actor Critic & On-policy & \\
\hline
ACER  &  Actor Critic with Experience Replay [@wang_sample_2017] & Model-free & Actor Critic & Off-policy & Not suited for discrete action spaces \\
\hline
TRPO  & Trust Region Policy Optimization [@trpo] & Model-free & Actor Critic & On-policy & \\
\hline
PPO  & Proximal Policy Optimization [@ppo] & Model-free & Actor Critic & On-policy & \\
\hline
DQN & Deep Q Networks [@dqn] & Model-free & Value-based & Off-policy & Not suited for continuous action spaces; Suffers from overestimation bias\\
\hline
Double DQN & Deep Q Networks [@van_hasselt_deep_2015] & Model-free & Value-based & Off-policy & Not suited for continuous action spaces \\
\hline
DDPG & Deep Deterministic Policy Gradient [@ddpg] & Model-free & Actor Critic & Off-policy & Not suited for discrete action spaces\\
\hline
TD3 & Twin Delayed DDPG [@td3] & Model-free & Actor Critic & Off-policy & Not suited for discrete action spaces \\
\hline
SAC & Soft Actor Critic [@sac] & Model-free & Actor Critic & Off-policy & Not suited for discrete action spaces\\
\hline
IMPALA & Importance Weighted Actor Learner [@impala] & Model-free & Actor Critic & Off-policy & \\
\hline
\end{tabular}
\caption{A survey of popular deep RL algorithms. With the plethora of deep RL algorithms, this table aims to show some of the most common ones. Model-free algorithms are in the majority in this table as the model-free approach has garnered more attention in recent years. Model-based approaches like MCTS, which is the basis for AlphaZero, are still able to achieve state of the art performances on certain tasks, so they should not be overlooked. Despite there being a range of different algorithms in this table, many of them are closely related to one another. E.g. Double DQN makes a small adjustment to DQN; PPO is an evolution of TRPO; TD3 is an evolution of DDPG; and, A2C is marginally different from A3C. These small differences, however, can result in large change in agent performance. All deep RL algorithms have their respective weak points, and it is impossible to know how algorithms will perform on a task until they are tuned and evaluated. As a deep rl practitioner, the best thing to do then is to find a range of algorithms that can work on your problem's state and action space, tune the algorithms, and see how they perform.}
\end{center}
```
# Examples
...
The use of a recurrent neural network (RNN) architecture instead of the multi-layer perceptron (MLP) used here would open the door for the policy to reflect the non-autonomous dynamics more directly.
...
The science of ecological management and quantitative decision-making has a long history [e.g. @Schaefer1954; @Walters1974] and remains an active area of research [@Possingham2006; @Fischer2009; @Polasky2011].
...
Alternative models can easily be implemented in simulations, allowing an agent to train against both contingencies.
A successful agent will learn a policy that works well in both scenarios.
This is analogous to classical approaches for integrating over parameter or model uncertainty [e.g. @Walters1981; @Ludwig1982],
...
The flexibility of the approach makes it relatively straightforward to include realistic biology and environmental science in framing an RL problem, but training an RL agent to reach an acceptable level of performance is another task.
...
Finding acceptable policies even in relatively simple problems may prove beyond the reach of existing algorithms and available computing power.
In seeking either to adopt or assess a deep RL-based approach to an ecological issue, we highlight three key issues that are important to bear in mind: instability, interpretability, and reward design.

<!-- reason 1: instability-->
One leading challenge for deep RL today is the problem of instability.
RL agents trained on narrowly defined environments may perform poorly even when the environment is altered in seemingly trivial way, such as a re-scaling of parameters.
To avoid this, researchers should seek to both train and evaluate agents against a range of environments representing different parameters or processes dynamics.
Stable performance can be challenging even when using the same environment.
For a range of reasons, including high variance [@cite] and the exploration vs. exploitation trade-off [@cite], model-free and model-based algorithms tend to have unstable performance on environments with non-tabular state-action spaces [cite RL that matters].
For instance, it is not uncommon to observe vastly different behavior when an agent is initialized with different random seeds [cite ibid].
Such unstable behavior can often be avoided by alternate selection of hyperparameters, so researchers should consider stability as well as performance during hyperparameter selection.
The choice of RL algorithm, design of the reward function, and duration of training can also influence stability.


<!--
Under some conditions, RL agents can prove adept at _transfer learning_, in which an agent trained in one environment performs well under a different environment.
-->
<!-- if we're really recommending 'testing' for stability, maybe we need to demo that in the appendix-->

<!-- reason 2: interpretability -->
Interpretability is a common challenge ot the adoption of most ML methods [@cite]. 
...
As our examples have illustrated, it may be possible to gain intuition on an agent's behavior by visualizing the action selected against each possible state, though this becomes more difficult in large problems.
Another challenge is that Deep RL algorithms are very sample inefficient, all though progress is being made to improve this [@cite].  
-->


<!-- reason 3: reward design -->
An issue that is central to any applied optimization problem is using the right objective.
There have been many examples in which explicit reward design goes very wrong [cite].
...
RL researchers often introduce modified reward functions during training that help an agent perform better on the original objective [@cite].
This approach is particularly common when rewards are sparse, such as in a game of Chess where the only objective that counts is whether or not you win, but during training, an RL algorithm may reward the agent for intermediate steps such as capturing a piece.
...
-->
<!-- reason 4/additional reasons, like sparse rewards -->
Ultimately, we anticipate that getting deep RL to work well on fully detailed conservation decision-making problems will be challenging.
...
Leading developers of new algorithms for deep RL benchmark the performance of their algorithms against such problems in part because simulations of games and robotic movement are readily available through open source frameworks designed to interface with RL algorithms [@gym], and performance can easily be compared against alternatives on public leaderboards [@gym].
