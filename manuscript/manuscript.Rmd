...
Over the course of an episode, an *agent*, which is often a computer program, continuously interacts with the environment. 
At each time step, the agent performs an *action*, and as a result of this action, the environment will determine the next *state* and numerical *reward* that the agent will observe.
...
Once the agent comes across a highly rewarding sequence of states and actions, the agent will reinforce this behavior so that it is more likely for the agent to *exploit* the same high reward trajectory in future episodes.
Throughout this process, the agent's behavior is codified into what is called a *policy*, which is a mapping of states to actions.

To solve the RL problem, algorithms either take a *model-free* or *model-based* approach. 
The distinction is that *model-free* algorithms do not have access to a model of the environment; yet, *model-based* algorithms use a model of the environment to achieve the RL objective.
...
Amongst model-free algorithms, there is divergence around what functions the agent is attempting to learn.
...
*Actor-critic* algorithms attempt to learn both a policy function and a value function, whereby the value function, which is learned separately from the policy, is used to inform the agent on the goodness of a selected action.
There is a handful of advantages and disadvantages for each of these approaches that we will not go in depth -- for more, see [@CITE]; yet, the main point of distinction comes from the bias-variance trade-off.
Policy gradients tend to struggle with high variance, value-learning tends to be biased while actor-critic methods balance these two concerns.
