...
abstract: |
  ...
  the required expertise and computational cost may place these applications may be beyond the reach of all but international technology firms.
  Such an advent would represent a sea-change not only in what problems we can solve, but what stakeholders are involved in solving them.
...
nocite: |
    @A2C, @trpo, @ddpg, @TD3, @impala, @hafner_learning_2019, @weber_imagination-augmented_2018
header-includes:
  - \usepackage{amsmath}
  - \usepackage{amssymb}
#  - \usepackage{lineno}
...
Precise estimates of computational resources used in that research are difficult to establish, but previous estimates benchmarked against commercially avilable cloud computing platforms place the training of a single model at over $30 million [@alphacost],
and many realistic ecological problems will involve greater complexity than board games of two players on a 19 x 19 grid [@alphago].
...
Levins esposed simple mechanistic models which satisfy the goals of being both _realistic_ and _general_. 
...
Prior work sometimes overlooks this essential step, assuming that whatever behavior an RL agent produces is sufficiently optimal or otherwise an accurate proxy for behavior arising from animal learning or evolution. 
As our evaluations will illustrate, such an assumption can be quickly misleading. 
Second, these models are already widely studied and will be familiar to many readers: @Schaefer1954 is a staple of fisheries management textbooks and practice, with over 2800 citations, while @May1977 has become a canonical model of threshholds and tipping points which still continues to dominate how many ecologists think about these phenomena [@Scheffer2009].  
Many readers can thus benefit from exisiting knowledge and intutition about the behavior and implications of these models in interpreting the performance of deep RL, something that would not be possible with a more complex model. 
It would be unwise to consider applications of RL on more complex bespoke models without first analyzing these classic and well-understood cases.
...
Our implementations of these models in two python modules published to `PyPi` code archive include many such variations which represent open problems for RL.
...
We do recognize the need for more complex, "precise" models to derive quanitatively precise management policy in specific management problems such as fisheries, forestry, or climate change mitigation efforts. 
The examples here should provide ecologists with the greater background to engage in the collaborative development of such methods, even as they also highlight challenges which ecological problems pose to existing techniques.
Even the simple models we consider here contain many sources of complexity -- significant stochasticity, non-linear dynamics, hysteresis -- that are likely commonplace to ecological problems but have not been well represtented in RL research to date. 
Such work is beyond the scope of any one paper: it will require not only more computational resources, but more collaborations between large teams of ecologists and computer scientists.  
...
Even on relatively uncomplicated environments, training an RL agent can be more challenging than expected due to an entanglement of reasons like hyperparameter instability and poor exploration that can be very difficult to resolve [@drlthatmatters; @berger-tal_exploration-exploitation_2014].
