...
In this paper, we draw on examples from fisheries management, ecological tipping points, and wildfire spread to illustrate how deep Reinforcement Learning techniques can successfully discover optimal solutions to previously solved management scenarios and discover highly effective solutions to unsolved problems.
...
Some familiarity with the design and potential application of RL is important not only to researchers interested in developing these methods to solve new problems, but is also important to any ecologist or manager 
We discuss of how an open, transparent and reproducible approach can help mitigate some concerns, while also offering a more effective interface between teams of researchers from both ecological and computer sciences.
...
We further include implementations of three fully featured simulation modules following current leading standards, which would allow engineers and computer scientists already working in RL to test their agents against both solved and unsolved problems in conservation management.
...
Classic approaches require 


Reinforcement learning works much like human learning -- the basic techniques of RL have been used to train animals to perform complex tasks long before they were used to train computational neural networks [@behavioral_psych].
Ecological decision-making has its own rich and rapidly expanding suite of complex simulators:
SORTIE in forest ecosystems [@sortie], EcoPath with EcoSim in marine systems [@ecopath], GCM models in climate [@gcm], 
...
Development of any deep reinforcement learning application can be divided into two components: an *environment* and an *agent*.
...
An *agent* is typically a computational neural network based on an appropriate RL algorithm that may have been 'trained' on one or more environments.
...
In principle, there are virtually no restrictions on what comprises a state: it may be continuous or discrete, completely or only partially observed, one or many dimensions.
...
The `gym` framework is now recognized by all major software frameworks for writing RL algorithms (see below, 'Deep Reinforcement Learning Frameworks').
...
We use the `gym` framework to turn several existing ecological models into valid environmental simulators that can similarly be used in any Reinforcement Learning framework.
...
The design of RL algorithms has been and remains an area of intense research and rapid development [Table 1].
...
Considerations in environmental design, such as the normalization of state and/or action spaces, can significantly improve performance of many RL algorithms on a diverse range of problems without requiring extensive tuning.


Somewhat confusingly, frameworks will often refer to a trained agent as a "model", while also referring to the approaches used to implement and train the agent as "model free" because the agent does not attempt to form an internal model of the environmental process.  We will attempt to mitigate this confusion by referring to the software component used to predict actions as an "agent", and reserving the use of "model" to refer to the ecological model(s) embedded in the environmental simulator.  




A rapidly growing set of material for learning about deep reinforcement learning is now freely available.  
...
-   model (underlying ecological model used to drive step() method in environment)

-   model (SB3 term  for "agent")

-   predict (SB3 term to return agent's action given an observation)

-   predict (ecological model which generates probabilistic forecast of future states)
