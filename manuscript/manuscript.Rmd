...
The flexibility of the approach makes it relatively straightforward to include realistic biology and environmental science in framing an RL problem, but training an RL agent to reach an acceptable level of performance can often be more challenging than expected.
...
Currently, deep RL agents struggle to generalize their skills [@pmlr-v97-cobbe19a].
This inability can affect performance during training and at evaluation. 
For instance, training an agent on an environment that demands learning an assortment of skills is typically much more difficult than training an agent to learn one specific skill [@teh_distral_2017].
And, at evaluation, if you give an agent a novel task, including tasks that are closely related to the examples that the agent saw in training, the agent can easily fail to generalize its learned behavior to this new situation [@pmlr-v97-cobbe19a].
These failures are all due to the brittleness of deep neural networks. 
Across deep learning, neural networks tend to perform exceptionally well on narrowly defined problems, yet neural networks show great fragility when given new situations or problem statements that require flexibility [@cogswell_reducing_2016].
The ability to adapt to new situations is critical for any decision-making problem in the real world, so the current lack of this ability in deep RL may limit deep RL's effectiveness to conservation problems.
...
Deep RL agents are also difficult to interpret [@heuillet_explainability_2021].
Deep neural networks are commonly referred to as black box models, since it is challenging to fully explain why a deep neural network produces a specific output from a given input [@castelvecchi_can_2016].
For deep RL, this means that we can rarely provide a meaningful explanation for why any agent selects an action.
...
When going from a simulator to the real world, the real world may introduce novel situations that a simulator-trained agent is not familiar with, and since we can not anticipate how an agent will respond to new situations, there is an inherent risk in letting RL agents make decisions in the real world. 

<!-- reason 3: reward design -->
And lastly, a fundamental issue for all applied optimization problems is using the right objective.
There have been many examples in which explicit reward design produces unanticipated agent behavior that is highly undesirable [@hadfield-menell_inverse_2020].
Thus, for ecological systems, which are typically quite difficult to control, specifying a reward function could easily produce behavior that opposes the intents of the RL practitioner.
To guide agents to learn desirable policies, RL researchers often *shape* the reward function so that the agent will more readily learn certain behaviors [@reward_shaping].
...
Frequently, when researchers do not shape the rewards on sparse reward problems, the RL agent learns nothing, but reward shaping can still backfire by producing unintended agent behavior [@hadfield-menell_inverse_2020]. 
