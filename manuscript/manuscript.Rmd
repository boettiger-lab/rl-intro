...
In table 1, we provide some of the commonly used RL algorithms and delineate some of their important characteristics.
...
<!-- ML: I think it might be confusing to introduce RNNs, maybe just say different architectures, maybe make a footnote.  CB: agree, revised above-->
...
<!-- Marcus is working on tightening up these next few paragraphs, precisely which issues actually get highlighted TBD-->
In seeking either to adopt or assess a deep RL-based approach to an ecological issue, we highlight three key issues that are important to bear in mind: generalizability, interpretability, and reward design.


<!-- reason 1: generalizability-->
<!-- ML: pivoting this to generalizability but still rewrote past paragraph which might fit in the appendix somewhere-->
Currently, deep RL agents struggle to generalize their skills [@generalization].
This inability can affect performance during training and at evaluation. 
For instance, training an agent on an environment that demands learning an assortment of skills is typically much more difficult than training an agent to learn one specific skill [@CITE].
And, at evaluation, if you give an agent a novel task, including tasks that are closely related to the examples that the agent saw in training, the agent will likely fail to generalize its learned behavior to this new situation [@CITE].
These failures are all due to the brittleness of deep neural networks. 
Across deep learning, neural networks tend to perform exceptionally well on narrowly specified problems, yet neural networks show great fragility when given new situations or problem statements that require flexibility [@CITE].
The ability to adapt to new situations is critical for any decision-making problem in the real world, so the current lack of this ability in deep RL may limit deep RL's effectiveness to conservation issues.

<!-- 
A significant current challenge in deep RL is the problem of instability.
For a range of reasons, including high variance of returns and the exploration vs. exploitation dilemma, deep RL algorithms tend to have unstable performance on environments with large state-action spaces [@drlthatmatters].
For example, @drlthatmatters show that initializing the same agent with different random seeds can lead to significant differences in agent performance.
Such unstable behavior can be avoided by finding a robust set of hyperparameters or using algorithms that are designed to have greater reliability [@qprop], thus researchers should consider stability as well as performance during hyperparameter tuning and algorithm selection.
-->
...
Compounded with their difficulties to generalize, deep RL agents also suffer from an inability to be fully interpreted [@cite].
Deep neural networks are commonly referred to as black box models, since it is not currently possible to explain why a deep neural network produces a specific output from a given input.
In the context of deep RL, this means that we cannot explain why an agents makes a decision besides that the agent is attempting to maximize cumulative rewards.
The lack of explainability dovetails dangerously with generalizability concerns. 
Because we are not able to interpret deep RL agents, we are not able to anticipate how an agent will behave when in an unfamiliar situation.
Despite limited progress to make neural networks more explainable, interpretability will likely be a concern that limits adoption.
...
A fundamental issue for all applied optimization problems is using the right objective.
There have been many examples in which explicit reward design produces unanticipated agent behavior that is highly undesirable [@CITE].
For ecological systems, which are typically very complex and difficult to fully understand, it seems that bad reward design is quite likely.
To avoid these undesirable outcomes, RL researchers often *shape* the reward functions so that the agent can more easily learn a good-performing policy [@cite].
This approach is particularly common when rewards are *sparse*, such as in a game of Chess where the objective is to win and there are no true immediate rewards.
In this case, one might shape the reward function so that the agent is awarded for capturing the opponent's pieces, but this could easily lead to a policy where the agent focuses on capturing pieces instead of winning the game.
Since benefits in conservation typically come long after their corresponding actions, sparse reward.
...
Leading developers of new algorithms for deep RL benchmark the performance of their algorithms against such problems in part because simulations of games and robotic movement are readily available through open source frameworks designed to interface with RL algorithms [brockman2016], and performance can easily be compared against alternatives on public leaderboards [brockman2016].
