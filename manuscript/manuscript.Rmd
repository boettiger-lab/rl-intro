...
This inability can be observed in training: if you train an agent on an environment that demands learning a wide range of tasks, the agent will likely fail to converge to an optimal policy [@CITE].
And, during evaluation, if you give an agent any novel task, including tasks that are closely related to the examples that the agent saw in training, the agent will likely fail to generalize its policy to this new situation [@CITE].
...
Interpretability is an issue that may hinder the adoption of deep RL methods [@cite].
