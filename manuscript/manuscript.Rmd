...
Across deep learning, neural networks tend to perform exceptionally well on narrowly defined problems, yet neural networks show great fragility when given new situations or problem statements that require flexibility [@CITE].
...
Deep RL agents are also very difficult to interpret [@cite].
...
Frequently, when researchers do not shape the rewards on sparse reward problems, the RL agent learns nothing [@CITE], but reward shaping can still backfire by producing unintended agent behavior [@CITE]. 
