...
One feature that most distinguishes deep RL from other approaches to decision theory (such as optimal control) is its flexibility and scope.
While optimal control approaches typically require extensive simplifying assumptions such as discrete states or actions, deterministic dynamics or short time horizons, deep RL is scoped more ambitiously.
Large, complex problems are still challenging to RL, and existing RL approaches can easily struggle to solve problems that can already be solved much more efficiently by other means.
Nevertheless, deep RL approaches have proven successful on problems that classical approaches could not solve despite immense computational resources -- e.g. deep RL  [@DQN, @alphago].
Our previous examples have focused on relatively simple problems that can also be addressed at least approximately by conventional methods, and also permit RL-based solutions to be found relatively efficiently on commodity hardware.
Yet, we also have developed 
...
In the case that someone trains an agent to perform well on a realistic environment, there will still be a range of open questions in using this agent to inform decision-making: 
