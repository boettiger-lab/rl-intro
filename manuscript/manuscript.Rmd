...
An *environment* is typically a computer simulation, though it is possible to use the real world as the RL environment [@robotics; @offlinesurvey].
...
The trade-off between these approaches is that when it is possible to quickly learn a good model or the model is already known, model-based algorithms tend to require much less interaction with the environment to learn good-performing policies. 
Yet, oftentimes learning a model of the environment is very difficult, and in these cases, model-free algorithms tend to outperform.
