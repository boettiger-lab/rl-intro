...
While conservation objectives are often framed in broad, simple terms, the potential action space is often huge: conservation efforts can be implemented in a wide variety of ways, and the most effective policies may vary in both space and time. <!-- Millie: can we say something more precise about 'simple terms' or add some citation here?-->
...
However, successfully applying RL to complex ecological problems is no easy task.
...
<!-- something about several technical caveats: instability, interpretability reward design-->

<!-- reason 1: instability-->
One leading problem with deep RL is instability.
...
<!--CB: I think we can say more about how to diagnose this problem if not fix it.
    Likewise should address the importance of assessing robustness across multiple models/environments, probably at least as important an issue as robustness to nuciance hyperparameters like random seed.
    -->

<!-- reason 2: interpretability --> 
In the context of conservation, we anticipate that there will be a host of specific challenges to using deep RL methods.
Interpretability is likely to be the biggest barrier to adoption. <!-- don't need to rank them -->
