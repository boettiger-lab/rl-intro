...
In behavioral ecology studies, RL agents are used to represent organisms.
Assuming that animals attempt to maximize some cumulative reward like RL agents, the authors of these studies draw conclusions about animal behavior and evolution by observing how the RL agents perform.
When applying deep RL to conservation management, we do not make the assumption that the behavior of an RL agent is a good substitute for the behavior of an animal.
For conservation management, we instead propose using RL as a tool to search for solutions to unsolved decision-making problems. 
