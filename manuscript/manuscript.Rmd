...
<!-- citation suggestion for declining biodiversity  -->
...
<!-- suggestion: "...tackle our most pressing environmental challenges" -->
...
<!-- really like this- super clear and helpful! -->
...
<!-- another widely used model is ED2 (ecosystem demography model) - I think more common than SORITE? https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2008JG000812 -->
...
<!-- I think there is a missing the connection between the examples and needing more sophisticated methods for decision making below. i.e. "As we develop more complex process based models (such as Ecosim and ED2), methods that allow us to effectively integrate these high dimensional models into decision making processes will be critical to ensuring their policy relevance." (or something like that, then go on to NN ML)-->
...
We intend to continue to expand this library of ecological management benchmarks with the hope both of challenging researchers already working in RL to tackle some of these pressing issues and for ecologists and environmental scientists to continue to refine these benchmarks into more realistic ecological simulations.
 ...
 <!-- I like the summary but could maybe be less specific> "We start with a simple example from fisheries management that has a known optimal solution, proving that with no prior knowledge of underlying model dynamics RL agents can find a close to optimal solution. We go on to show that RL can outperform rule-of-thumb solutions to management scenarios not amenable to classic optimization methods such as dynamic programming. Our examples demonstrate that..."  -->
...
<!-- maybe cite Caleb's paper here? -->

<!-- these last few sentences almost feel like discussion points. I'm torn though, because it it nice to have them in the intro. Maybe condense into 1 sentence at the end of you "summary" paragraph of the bulk of the paper above. for example: "We also explore the potential pitfalls of RL-based solutions to global environmental change problems. We show how, and why, these challenges are not only technical, but include issues of ethics and power, especially when the algorithms or data are proprietary." then add a longer discussion towards the end of the paper-->
...
<!-- something to introduce this section/transition from the intro might be helpful: "We start with a brief overview of RL problems and terminology:" -->
...
<!-- ^ this is great! I do think keeping a table of these terms would be helpful (and refering to that table in the text here) -->
...
By contrast, *Policy gradient* algorithms exclusively learn the policy.
...
In contrast to classical approaches, such as dynamic programming, there are few restrictions on what comprises a state or action, at least in principle.
...
<!-- maybe consider some combined conceptual figure/table?  But overall I really like the table and think it will be helpful for readers -->
...
Algorithms are playing increasingly influential decision-making roles in sectors such as policing, finance, and employment, raising concerns about the ethics, biases, and power entrenched in both the algorithms and their outputs [@kalluri2020]. As algorithms play a greater role in environmental and conservation policy, will similar concerns arise? While this question has garnered  recent interest in the literature [@wearn2019; @adams2019; @scoville2021] the use use of decision making algorithms in environmental applications has largely been confined to the context of classification algorithms (i.e. to identify wildlife poaching [@adams2019] or illegal fishing behavior [@mcdonald2021]). Increasingly, applications of AI to meeting sustainable development goals [@], prioritizing protected area locations [@], and predicting However, good intentions are not sufficient to ensure equitable outcomes. Interdisciplinary collaborations with experts in algorithmic governance and ethics will be important ensure that applications of AI to ecological challenges are applied responsibly. 
