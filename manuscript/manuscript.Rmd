...
The amount of time needed for an agent to learn high reward yielding behavior cannot be predetermined and depends on a host of factors including the amount of dedicated compute, complexity of the environment, complexity of the agent and more [@cite].
...
Since the optimal hyperparameters vary across environments and can only be found through exhaustive search [@cite], it is necessary to find a good-performing set of hyperparameters in a process called hyperparameter tuning.
...
After the agent observes the state at time t, $s_t$, the agent selects an action, $a_t$, according to its policy, $\pi$, which maps states to a probability distribution over actions, $\pi: \mathcal{S} \rightarrow p(\mathcal{A} \mid \mathcal{S})$.
...
While there are RL methods to deal with infinite horizon cases, we will only present methods for finite horizon or episodic MDP and POMDPs.
...
There is a handful of advantages and disadvantages for each of these approaches that we will not get into in depth -- for more, see the appendix; yet, the main point of divergence comes from the bias-variance trade-off [@CITE].
...
This *exploration-exploitation dilemma* is an open research problem [@CITE], and current deep RL algorithms take a range of approaches towards this dilemma -- in the appendix, we discuss some of the more specific exploration algorithms.
Yet, generally, the manner in which RL algorithms explore depend on whether the algorithms are *on-policy* or *off-policy*.
The distinction between on and off-policy exploration comes from what policy the agent is using to select actions during training.
...
\hline
MBPO & Model-based Policy Optimization [@mbpo] & Model-based & On-policy & Continuous & Continuous\\
\hline
MCTS & Monte Carlo Tree Search [@suttonbarto] & Model-based & Either & & \\
\hline
A2C  & Advantage Actor Critic [@a2c] & Model-free & On-policy& &\\
\hline
A3C  & Asynchronous A2C & Model-free [@a3c] & On-policy & &\\
\hline
TRPO  & Trust Region Policy Optimization [@trpo] & Model-free & On-policy & &\\
\hline
PPO  & Proximal Policy Optimization [@ppo] & Model-free & On-policy & &\\
\hline
DQN & Deep Q Networks [@dqn] & Model-free & Off-policy& &\\
\hline
DDPG & Deep Deterministic Policy Gradient [@ddpg] & Model-free & Off-policy & &\\
\hline
TD3 & Twin Delayed DDPG [@td3] & Model-free & Off-policy & &\\
\hline
SAC & Soft Actor Critic [@sac] & Model-free & Off-policy & &\\
\hline
IMPALA & Importance Weighted Actor Learner [@impala] & Model-free & Off-policy & &\\
\hline
APE-X & Distributed Prioritized Experience Replay [@apex] & Model-free & Off-policy & & \\
\hline
\end{tabular}
\end{center}
```
