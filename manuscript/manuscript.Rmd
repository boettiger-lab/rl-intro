...
Thus, for ecological systems, which are typically difficult to control and fully understand, specifying a reward function could easily produce behavior that opposes the intents of the RL practitioner.
To guide agents to learn desirable policies, RL researchers often *shape* the reward function so that the agent will more readily learn a certain behavior[@cite].
...
Frequently, when researchers do not shape the rewards on sparse reward problems, the RL agent learns nothing [@CITE], but reward shaping can still produce unintended agent behavior [@CITE]. 
