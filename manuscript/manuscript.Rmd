...
Reinforcement Learning (RL), a machine learning technique in which a *software agent* is trained to select actions to maximize some cumulative reward, could hold significant untapped promise for solving complex ecological and conservation decision-making problems from wildfire management to biodiversity preservation to zoonotic pandemics.
Unlike the more widely recognized machine learning areas of *supervised* learning and *unsupervised* learning, RL focuses on solving *dynamic* problems which often require planning ahead in uncertain and changing environments.
...
In contrast to other ML methods, RL does not require massive data sets for training and can instead rely on simulators.
...
Moreover, no amount of data can train supervised or unsupervised ML approaches to predict a future which has no analog in the historical record.<!-- ML: potential counterpoint here of using synthetic data to train SL/USL algorithms-->
...
*Deep reinforcement learning*, which applies recent advancements from deep learning to reinforcement learning, has been able to solve previously intractable, high dimensional sequential decision making problems.
A strength of deep RL methods is that, given sufficient computational power, they can be easily applied to any decision-making problem that can be formulated as a *Markov-decision process* (MDP).
A Markov-decision process provides a standard ... (working on this)
...
Lacking the tractability of classical methods, an RL agent can appear to solve a wide class of scenarios with ease, but may fail unexpectedly under seemingly similar conditions [@henderson2019].
Despite recent progress, Deep RL is a young field, and stability and robustness of its methods remain largely open problems [@henderson2019]. 
...
Learning the advantage function is often preferred in practice as advantage estimators favorably have lower variance than state-value and Q-value estimators [cite schulman gae].
...
Once the transition model is known, the agent can then use the transition model to plan out what the optimal action is, often with Model Predictive Control [cite tassa 2012 MPC], or the agent can use the transition model to generate more samples to aid learning [cite Sutton dyna, alphago?]
.
In model-free RL, the agent does not attempt to learn a dynamics model and instead tries to either learn a parameterized policy, learn a value function, or try to learn both.
...
The benefit of off-policy learning is that the agent does not need to collect a sample every time the agent updates its behavior; instead, the agent can learn from past experience, which increases sample efficiency [@SAC].
...
Neural networks, which have the property of being universal function approximators [cite], are thus a powerful tool in RL since neural networks can be used to approximate policy and/or value functions which we do not know the form of.
Additionally, the ability of neural networks to autonomously devise low-dimensional representations of data is very beneficial when state observations are high dimension, for instance in the case of image-based observations.
...
\hline
MBPO [@MBPO] & Model-based Policy Optimization & Model-based & On-policy& &\\
\hline
MCTS & Monte Carlo Tree Search & Model-based & Either & & \\
\hline
A2C  [@A2C] & Advantage Actor Critic & Model-free & On-policy& &\\
\hline
A3C [@A3C] & Asynchronous A2C & Model-free & On-policy & &\\
\hline
TRPO [@TRPO] & Trust Region Policy Optimization & Model-free & On-policy & &\\
\hline
PPO [@PPO] & Proximal Policy Optimization & Model-free & On-policy & &\\
\hline
DQN [@DQN] & Deep Q Networks & Model-free & Off-policy& &\\
\hline
DDPG [@DDPG] & Deep Deterministic Policy Gradient & Model-free & Off-policy & &\\
\hline
TD3 [@TD3] & Twin Delayed DDPG & Model-free & Off-policy & &\\
\hline
SAC [@SAC] & Soft Actor Critic & Model-free & Off-policy & &\\
\hline
IMPALA [@IMPALA] & Importance Weighted Actor Learner & Model-free & Off-policy & &\\
\hline
APE-X [@APE-X] & Distributed Prioritized Experience Replay & Model-free & Off-policy & & \\
\hline
\end{tabular}
\end{center}
```
