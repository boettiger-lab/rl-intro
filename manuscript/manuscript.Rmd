...
Note that we have no way of knowing for sure if the RL quotas would have led to recovery, let alone a truly optimal harvest rate, because we can never know the "true model" of the Argentine hake dynamics.
We can confirm that fishery closures seen in the RL agent's solution is also considered optimal under the assumptions of constant escapement theory [@Reed1979] whenever the stock is below the biomass of maximum sustainable yield ($B_{MSY}$), and that most fisheries models of this stock [@ramlegacy] would suggest that the stock sizes observed in the latter two decades of the data are below that threshold.
