...
On top of the aforementioned distinctions of deep RL algorithms, there are more algorithms design choices that are made. 
A very significant consideration is deciding how the agent will explore the environment.
Another is whether the algorithm can work on a discrete, continuous or both a discrete and continuous state and action space.
...
However, unlike an analytic or SDP solution that is derived directly from the estimated model, a model-free deep RL algorithm makes no assumption about the precise functional form or parameter values underlying the dynamics -- it is in principle agnostic to the details of the simulation.

Here, we apply a version of the deep RL algorithm known as Twin Delayed Deep Deterministic Policy Gradient or more simply, TD3 [@TD3].
...
Because these simulations rarely reach stock sizes at or above carrying capacity, these larger stock sizes show a greater deviation from the optimal policy than we observe at lower stock sizes.
As intuition would suggest, it is possible to devise alternative training schemes which may do a better job sampling the simulation dynamics at high stock levels.
Training these agents in a variety of alternative contexts can improve there ability to generalize to other scenarios.


<!-- 
Add text about empirical example
-->
