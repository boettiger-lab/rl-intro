...
Supervised and unsupervised learning approaches require massive amounts of historical data to learn underlying patterns.  
Even when such data is eventually available, they may prove reliable for relatively static tasks such as species identification from audio or visual recordings, yet be a poor guide to a future climate we have never observed.  
...
To date, the uncertain and changing environments considered by RL research have largely been drawn from examples in robotic movement and games. 
_Nature_ recently featured RL agents achieving superhuman performance on classic Atari games, _Science_ described the success of Deepmind's AlphaZero against grandmasters in Go, Chess, and Shogi.
