...
The goal of RL is to train a software agent to maximize some objective in a dynamic environment.
To date, researchers have used RL to make a number of impressive achievements on decision-making problems that were previously thought to be unsolvable -- one notable example is Deepmind's AlphaGo agent which defeated a Go Grandmaster. 

An advantage of RL is that unlike other branches of ML, RL can rely entirely on simulators to achieve optimal performance.
Rather than seeking to displace process-based ecological models of the past century with opaque machines, we illustrate how we can use such mechanistic models to build realistic simulators that the RL agents will learn from.
